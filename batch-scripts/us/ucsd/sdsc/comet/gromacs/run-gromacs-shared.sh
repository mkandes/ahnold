...
# NAME
#
#     run-gromacs-shared.sh
#
# DESCRIPTION
#
#     An example Slurm [1] batch job script that is configured to run a
#     single-node, hybrid MPI + OpenMP molecular dynamics simulation 
#     with GROMACS [2] on SDSC's Comet supercomputer [3] in its 'shared' 
#     resource partition.
#
# USAGE
#
...
#SBATCH --partition=shared    # Request 18 cores on 1 'shared' node for
#SBATCH --nodes=1             # up to 2 hours.
#SBATCH --ntasks-per-node=18
#SBATCH --time=02:00:00

#SBATCH --job-name=gromacs-shared
#SBATCH --output=gromacs-shared.o%j.%N
...
time -p mpirun -np 3 gmx_mpi mdrun \
  -nb cpu \
  -pin on \
  -resethway \
  -noconfout \
